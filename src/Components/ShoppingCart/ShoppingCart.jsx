...
const ShoppingCart = ({close}) => {
    ...
    return (
        <div>
            { confirm  ? <TableModal close={close} name={'Сделать Заказ!'} onClick={close}/> :
                <div>
                        ...
                        <div className={styles.cart__list}>
                            <ul className={styles.cart__ul}>
                                {filterCategory.length === 0 ?
                                    <h3 style={{marginBottom: '20px'}}>Ничего не
                                        найдено!</h3> : filterCategory.map(item => (
                                    ...
                                    selectedProduct.map((item, index) => (
                                        <li key={index}>
                                            <div className={styles.selectedWrap}>
                                                <span>{item.name}</span>
                                                <span>{item.price} сом <img onClick={() => deleteProduct(index)} className={styles.cross} src={cross} alt='cross'/></span>
                                            </div>
                                        </li>
                                    ))
                                )}
                        ...
                        <div className={styles.totalPrice}>
                            <div className={styles.totalPriceWrap}>
                                <div className={styles.btnWrap}>
                                    <Button onClick={getConfirm} textButton={selectedProduct.length === 0 ? 'Закажи что нибудь' : 'Заказать Доставку'} w={157} h={47} bg={'#FF7400'}/>
                                </div>
